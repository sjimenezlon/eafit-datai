    ...
    lessons: [
      {
        ...
        content: [
          ...
          { type: 'theory', title: 'Paso a paso: Construir una subconsulta', content: 'Vamos a construir una subconsulta paso a paso. El objetivo es encontrar los productos que cuestan mas que el promedio de su categoria.<br><br><strong>Paso 1:</strong> Primero, entendemos los datos con un SELECT simple.<br><strong>Paso 2:</strong> Luego calculamos el valor que necesitamos (el promedio).<br><strong>Paso 3:</strong> Finalmente, usamos ese calculo como subconsulta en el WHERE.' },
          { type: 'code-example', title: 'Paso 1: Explorar los datos', content: 'Siempre empieza con un SELECT simple para entender que tienes:', code: "-- Paso 1: Ver los productos y sus precios\nSELECT nombre, precio, categoria_id\nFROM productos\nWHERE activo = 1\nORDER BY precio DESC\nLIMIT 10;" },
          { type: 'code-example', title: 'Paso 2: Calcular el valor de referencia', content: 'Ahora calculamos el precio promedio general:', code: "-- Paso 2: Cual es el precio promedio?\nSELECT ROUND(AVG(precio), 0) AS precio_promedio\nFROM productos\nWHERE activo = 1;\n-- Resultado: un valor como ~2,086,000" },
          ...
          { type: 'tip', title: 'Regla de oro para subconsultas', content: '<strong>Siempre construye de adentro hacia afuera.</strong> Primero escribe y prueba la subconsulta sola. Cuando funcione, envuelvela en la consulta exterior. Esto evita errores y te ayuda a entender cada parte.' },
        ],
        exercises: [
          {
            id: 'e-1-1-1',
          ...
          {
            id: 'e-1-1-3',
            instruction: 'Encuentra los productos que tienen una calificacion promedio de resenas superior al promedio general de calificaciones. Muestra el nombre del producto y su calificacion promedio (redondeada a 1 decimal). Ordena de mayor a menor.',
            initialQuery: '-- Productos con resenas por encima del promedio general\n',
            hints: [
              'Necesitas calcular el AVG(calificacion) por producto y compararlo con el AVG(calificacion) general de la tabla resenas',
          ...
          {
            id: 'e-1-1-4',
            instruction: 'Encuentra los proveedores que suministran al menos un producto cuyo precio sea mayor a 3,000,000. Muestra nombre del proveedor, pais y calificacion. Usa una subconsulta con IN.',
            initialQuery: '',
            hints: [
      ...
      },
      {
        ...
        content: [
          ...
          { type: 'theory', title: 'Paso a paso: Construir una tabla derivada', content: 'Las tablas derivadas se construyen de adentro hacia afuera. Vamos a crear un reporte que muestre los clientes con mas pedidos y su gasto total.<br><br><strong>Paso 1:</strong> Crear el resumen interno (la subconsulta).<br><strong>Paso 2:</strong> Verificar que la subconsulta funciona sola.<br><strong>Paso 3:</strong> Usarla como tabla en el FROM de la consulta exterior.' },
          { type: 'code-example', title: 'Paso 1: Crear el resumen interno', content: 'Primero, calculamos las metricas que necesitamos por cliente:', code: "-- Paso 1: Resumen de pedidos por cliente (esto sera nuestra tabla derivada)\nSELECT\n  cliente_id,\n  COUNT(*) AS num_pedidos,\n  SUM(total) AS gasto_total,\n  ROUND(AVG(total), 0) AS ticket_promedio\nFROM pedidos\nWHERE estado != 'Cancelado'\nGROUP BY cliente_id;" },
        ...
        exercises: [
          ...
          {
            id: 'e-1-2-2',
            instruction: 'Usando una tabla derivada, encuentra el producto mas vendido (mayor cantidad total) de cada categoria. Muestra el nombre de la categoria, nombre del producto y la cantidad total vendida. (Pista: primero calcula las ventas por producto, luego encuentra el maximo por categoria).',
            initialQuery: '-- Producto top por categoria usando tabla derivada\n',
            hints: [
              'La tabla derivada debe calcular SUM(dp.cantidad) agrupada por producto_id y categoria_id',
              'Luego necesitas filtrar para quedarte solo con el producto de mayor venta por categoria. Puedes usar otra subconsulta o simplemente ordenar y analizar.',
          ...
          {
            id: 'e-1-2-3',
            instruction: 'Crea un reporte que muestre por cada proveedor: nombre, numero de productos activos, precio promedio de sus productos, y el total de unidades vendidas de sus productos. Usa una tabla derivada para las ventas. Ordena por total de unidades vendidas descendente.',
            initialQuery: '',
            hints: [
      ...
      },
      {
        ...
        content: [
          ...
          { type: 'code-example', title: 'Paso 2: Agregar segmentacion', content: 'Ahora creamos los segmentos basados en total_compras y tipo:', code: "-- Paso 2: Segmentar cada cliente\nSELECT\n  nombre,\n  tipo,\n  total_compras,\n  CASE\n    WHEN tipo = 'VIP' AND total_compras >= 10000000 THEN 'Champion'\n    WHEN tipo IN ('VIP', 'Premium') THEN 'Leal'\n    WHEN total_compras >= 5000000 THEN 'Potencial Alto'\n    WHEN total_compras >= 1000000 THEN 'Promesa'\n    ELSE 'Nuevo/Bajo'\n  END AS segmento_rfm\nFROM clientes\nORDER BY total_compras DESC;" },
          { type: 'code-example', title: 'Paso 3: Reporte agrupado', content: 'Finalmente, contamos cuantos clientes hay en cada segmento:', code: "-- Paso 3: Reporte de distribucion por segmento\nSELECT\n  CASE\n    WHEN tipo = 'VIP' AND total_compras >= 10000000 THEN 'Champion'\n    WHEN tipo IN ('VIP', 'Premium') THEN 'Leal'\n    WHEN total_compras >= 5000000 THEN 'Potencial Alto'\n    WHEN total_compras >= 1000000 THEN 'Promesa'\n    ELSE 'Nuevo/Bajo'\n  END AS segmento,\n  COUNT(*) AS num_clientes,\n  ROUND(AVG(total_compras), 0) AS compra_promedio,\n  SUM(total_compras) AS compras_totales\nFROM clientes\nGROUP BY segmento\nORDER BY compras_totales DESC;" },
          { type: 'tip', title: 'CASE WHEN + GROUP BY = reportes poderosos', content: 'Combinar CASE WHEN con GROUP BY es una de las tecnicas mas usadas en analisis de datos. Te permite crear <strong>reportes de distribucion</strong> personalizados: cuantos clientes, productos o pedidos caen en cada categoria que tu defines.' },
        ],
        exercises: [
          {
            id: 'e-1-3-1',
          ...
          {
            id: 'e-1-3-3',
            instruction: 'Crea un reporte de rendimiento por metodo de pago. Para cada metodo_pago de la tabla pedidos, muestra: el metodo, numero de pedidos, total vendido, y una columna "rendimiento" que diga "Excelente" si el promedio por pedido > 5M, "Bueno" si > 2M, o "Regular" en otro caso. Excluye pedidos cancelados.',
            initialQuery: '-- Reporte de rendimiento por metodo de pago\n',
            hints: [
    ...
    lessons: [
      {
        ...
        content: [
          ...
          { type: 'theory', title: 'Paso a paso: Verificar antes de modificar', content: 'La regla mas importante del DML: <strong>siempre verifica con SELECT antes de ejecutar UPDATE o DELETE</strong>. Veamos el flujo completo para actualizar precios de una categoria.' },
          { type: 'code-example', title: 'Paso 1: SELECT de verificacion', content: 'Primero veamos que filas vamos a afectar:', code: "-- Paso 1: SIEMPRE haz SELECT primero con el mismo WHERE\nSELECT id, nombre, precio, costo\nFROM productos\nWHERE categoria_id = 1 AND activo = 1;\n-- Revisa: son estas las filas que quieres modificar?" },
          ...
          { type: 'code-example', title: 'Paso 3: Verificar resultado', content: 'Despues del UPDATE, verificamos que los cambios sean correctos:', code: "-- Paso 3: Verificar que el cambio se aplico bien\nSELECT id, nombre, precio, costo,\n  ROUND((precio - costo) * 100.0 / precio, 1) AS nuevo_margen_pct\nFROM productos\nWHERE categoria_id = 1 AND activo = 1;" },
          { type: 'tip', title: 'Flujo seguro: SELECT -> UPDATE -> SELECT', content: 'En produccion, los profesionales siempre siguen este flujo:<br>1. <code>SELECT</code> con el WHERE para ver que filas se afectaran<br>2. <code>UPDATE/DELETE</code> con el mismo WHERE<br>3. <code>SELECT</code> de nuevo para confirmar los cambios<br><br>Mejor aun: usa <code>BEGIN</code> y <code>COMMIT</code> (transacciones) para poder hacer <code>ROLLBACK</code> si algo sale mal.' },
        ],
        exercises: [
          {
            id: 'e-2-1-1',
          ...
          {
            id: 'e-2-1-3',
            instruction: 'Gestiona el sistema de cupones: Inserta un nuevo cupon con id=11, codigo="EAFIT2025", tipo="porcentaje", valor=15, minimo_compra=500000, fecha_inicio="2025-03-01", fecha_fin="2025-06-30", usos_maximos=100, usos_actuales=0, activo=1. Luego muestra todos los cupones activos ordenados por fecha_fin.',
            initialQuery: '-- Gestion de cupones\n',
            hints: [
              'INSERT INTO cupones (id, codigo, tipo, valor, ...) VALUES (...)',
              'Despues haz SELECT * FROM cupones WHERE activo = 1 ORDER BY fecha_fin',
          ...
          {
            id: 'e-2-1-4',
            instruction: 'Simula una devolucion: Cambia el estado del pedido con id=3 a "Devuelto". Luego incrementa el stock de cada producto de ese pedido segun la cantidad comprada (usa los datos de detalle_pedidos). Finalmente muestra el pedido y los productos afectados con su nuevo stock.',
            initialQuery: '',
            hints: [
      ...
      },
      {
        ...
        content: [
          ...
          { type: 'tip', title: 'Vistas como capa de abstraccion', content: 'Las vistas son perfectas para:<br>- <strong>Dashboard:</strong> crea una vista con los JOINs complejos y luego filtra facilmente<br>- <strong>Seguridad:</strong> comparte la vista (sin datos sensibles) en vez de la tabla completa<br>- <strong>Mantenimiento:</strong> si cambia la logica, solo actualizas la vista, no cada consulta' },
        ],
        exercises: [
          {
            id: 'e-2-2-1',
          ...
          {
            id: 'e-2-2-2',
            instruction: 'Crea una vista llamada vista_dashboard_ventas que muestre por cada categoria: nombre de la categoria, numero de productos vendidos (distintos), total de ingresos, y calificacion promedio de resenas. Luego consulta la vista ordenada por ingresos descendente.',
            initialQuery: '-- Vista de dashboard de ventas por categoria\n',
            hints: [
              'Necesitas JOIN entre categorias, productos, detalle_pedidos y resenas. Usa LEFT JOIN para resenas porque no todos los productos tienen.',
          ...
          {
            id: 'e-2-2-3',
            instruction: 'Crea una vista llamada vista_inventario_alerta que muestre productos con stock menor a 10 unidades: nombre del producto, nombre de la categoria, stock actual, precio, y nombre del proveedor. Luego consultala para ver los productos criticos.',
            initialQuery: '',
            hints: [
      ...
      },
      {
        ...
        content: [
          ...
          { type: 'theory', title: 'Paso a paso: Comparar conjuntos de datos', content: 'Las operaciones de conjuntos son poderosas para responder preguntas de negocio como: "Quienes compran pero no dejan resenas?" o "Que productos son populares en compras Y en resenas?".<br><br><strong>Paso 1:</strong> Define cada conjunto (consulta) por separado.<br><strong>Paso 2:</strong> Verifica que ambos tengan las mismas columnas.<br><strong>Paso 3:</strong> Combina con la operacion adecuada.' },
          { type: 'code-example', title: 'Paso 1: Definir los dos conjuntos', content: 'Queremos comparar: productos comprados vs productos con resenas.', code: "-- Conjunto A: Productos que han sido comprados\nSELECT DISTINCT p.nombre\nFROM detalle_pedidos dp\nJOIN productos p ON dp.producto_id = p.id;\n-- Resultado: lista de productos comprados\n\n-- Conjunto B: Productos que tienen resenas\nSELECT DISTINCT p.nombre\nFROM resenas r\nJOIN productos p ON r.producto_id = p.id;\n-- Resultado: lista de productos con resena" },
          ...
          { type: 'tip', title: 'UNION vs UNION ALL', content: '<code>UNION</code> elimina duplicados (mas lento). <code>UNION ALL</code> mantiene todos los resultados (mas rapido). Usa <code>UNION ALL</code> cuando sabes que no habra duplicados o cuando los necesitas, por ejemplo para crear un log consolidado de diferentes tablas.' },
        ],
        exercises: [
          {
            id: 'e-2-3-1',
          ...
          },
          {
            ...
            hints: [
              ...
              'Primer SELECT: pedidos JOIN clientes. Segundo SELECT: resenas JOIN clientes. Luego UNION ALL.',
          ...
          {
            id: 'e-2-3-3',
            instruction: 'Encuentra las ciudades donde hay clientes que han hecho pedidos por mas de 5 millones PERO donde NO hay clientes de tipo VIP. Usa EXCEPT. Muestra solo el nombre de la ciudad.',
            initialQuery: '',
            hints: [
    ...
    lessons: [
      {
        ...
        exercises: [
          ...
          {
            id: 'e-3-1-2',
            instruction: 'Crea un ranking de clientes activos por ciudad segun sus ingresos mensuales. Muestra nombre, ciudad, ingresos_mensuales y el ranking dentro de su ciudad (1 = mayor ingreso). Usa ROW_NUMBER y filtra solo el Top 3 de cada ciudad usando una subconsulta.',
      ...
      },
      {
        ...
        content: [
          ...
          { type: 'theory', title: 'LAG paso a paso: Detectar patrones de transacciones', content: '<strong>Paso 1:</strong> LAG(monto) OVER (ORDER BY fecha) toma el monto de la fila anterior (ordenada por fecha).<br><strong>Paso 2:</strong> La primera fila no tiene anterior, asi que LAG devuelve NULL.<br><strong>Paso 3:</strong> Puedes dar un valor por defecto: <code>LAG(monto, 1, 0)</code> devuelve 0 si no hay fila anterior.<br><strong>Paso 4:</strong> Combinado con PARTITION BY, puedes comparar transacciones consecutivas <em>dentro de cada cuenta</em>.<br><br>Esto es fundamental para detectar anomalias: si una transaccion es 10x mayor que la anterior, podria ser fraude.' },
          { type: 'code-example', title: 'LAG con PARTITION BY: Patron por cuenta', content: 'Compara cada transaccion con la anterior de la misma cuenta:', code: "SELECT\n  cuenta_origen_id,\n  fecha,\n  tipo,\n  monto,\n  LAG(monto) OVER (PARTITION BY cuenta_origen_id ORDER BY fecha) AS monto_anterior,\n  CASE\n    WHEN LAG(monto) OVER (PARTITION BY cuenta_origen_id ORDER BY fecha) IS NULL THEN 'Primera'\n    WHEN monto > LAG(monto) OVER (PARTITION BY cuenta_origen_id ORDER BY fecha) * 5 THEN 'Anomalia!'\n    ELSE 'Normal'\n  END AS patron\nFROM transacciones\nWHERE estado = 'Completada'\nORDER BY cuenta_origen_id, fecha\nLIMIT 20;" },
          { type: 'code-example', title: 'FIRST_VALUE y LAST_VALUE', content: '<code>FIRST_VALUE</code> toma el primer valor de la ventana. <code>LAST_VALUE</code> toma el ultimo (ojo: necesita ROWS BETWEEN para funcionar bien).', code: "SELECT\n  nombre,\n  ciudad,\n  ingresos_mensuales,\n  FIRST_VALUE(nombre) OVER (PARTITION BY ciudad ORDER BY ingresos_mensuales DESC) AS mejor_ingreso_ciudad,\n  FIRST_VALUE(ingresos_mensuales) OVER (PARTITION BY ciudad ORDER BY ingresos_mensuales DESC) AS mayor_ingreso\nFROM clientes_banco\nWHERE estado = 'Activo'\nORDER BY ciudad, ingresos_mensuales DESC;" },
          ...
          { type: 'tip', title: 'Cuando usar cada funcion analitica', content: '<strong>LAG/LEAD:</strong> Comparar con filas vecinas (periodo anterior/siguiente).<br><strong>FIRST_VALUE/LAST_VALUE:</strong> Comparar con el mejor/peor del grupo.<br><strong>NTILE:</strong> Segmentar en percentiles o cuartiles.<br><strong>ROW_NUMBER/RANK:</strong> Rankings y Top-N.' },
        ],
        exercises: [
          {
            id: 'e-3-2-1',
          ...
          {
            id: 'e-3-2-2',
            instruction: 'Para cada transaccion completada de la cuenta 1, muestra la fecha, el monto, el monto de la transaccion anterior (LAG) y la diferencia entre ambos. Ordena por fecha. Usa LAG con PARTITION BY cuenta_origen_id.',
    ...
    lessons: [
      {
        ...
        content: [
          ...
          { type: 'theory', title: 'Construir un multi-CTE paso a paso', content: '<strong>Paso 1:</strong> Identifica las "capas" de tu analisis. Ejemplo: quieres saber que clientes de alto riesgo tienen prestamos en mora.<br><strong>Paso 2:</strong> Escribe el primer CTE con la capa mas basica (ej: clasificar clientes).<br><strong>Paso 3:</strong> Agrega un segundo CTE con la siguiente capa (ej: agregar datos de prestamos).<br><strong>Paso 4:</strong> En el SELECT final, combina los CTEs con JOIN.<br><br><strong>Regla de oro:</strong> Cada CTE debe hacer UNA sola cosa. Si un CTE hace dos cosas, dividelo en dos.' },
          { type: 'code-example', title: 'Multi-CTE paso a paso: Analisis de cartera', content: 'Construyamos un reporte de cartera en 3 pasos:', code: "-- CTE 1: Resumen por sucursal\nWITH cartera_sucursal AS (\n  SELECT\n    s.nombre AS sucursal,\n    s.ciudad,\n    COUNT(DISTINCT p.id) AS num_prestamos,\n    SUM(p.monto_aprobado) AS total_aprobado,\n    SUM(p.saldo_pendiente) AS total_pendiente\n  FROM sucursales s\n  JOIN clientes_banco cl ON cl.sucursal_id = s.id\n  JOIN prestamos p ON p.cliente_id = cl.id\n  GROUP BY s.id, s.nombre, s.ciudad\n),\n-- CTE 2: Calcular indicadores\nindicadores AS (\n  SELECT\n    *,\n    ROUND((total_aprobado - total_pendiente) * 100.0 / total_aprobado, 1) AS pct_recuperado,\n    ROUND(total_pendiente * 1.0 / num_prestamos, 0) AS pendiente_promedio\n  FROM cartera_sucursal\n)\n-- Paso final: Mostrar con ranking\nSELECT\n  sucursal,\n  ciudad,\n  num_prestamos,\n  total_pendiente,\n  pct_recuperado,\n  RANK() OVER (ORDER BY pct_recuperado DESC) AS ranking_recuperacion\nFROM indicadores\nORDER BY ranking_recuperacion;" },
          { type: 'theory', title: 'CTEs Recursivos: El concepto', content: 'Un <strong>CTE recursivo</strong> se referencia a si mismo. Es util para recorrer jerarquias (organigramas, categorias anidadas) o generar series.<br><br><strong>Estructura:</strong><pre>WITH RECURSIVE nombre AS (\n  -- Caso base (anchor)\n  SELECT ... \n  UNION ALL\n  -- Caso recursivo (se referencia a si mismo)\n  SELECT ... FROM nombre WHERE condicion_parada\n)\nSELECT * FROM nombre;</pre><br><strong>Importante:</strong> Siempre necesitas una condicion de parada para evitar bucles infinitos.' },
          { type: 'code-example', title: 'CTE Recursivo: Generar serie de meses', content: 'Genera una serie de meses para analisis temporal (util cuando algunos meses no tienen datos):', code: "-- Generar los ultimos 6 meses\nWITH RECURSIVE meses AS (\n  SELECT date('2024-07-01') AS mes\n  UNION ALL\n  SELECT date(mes, '+1 month')\n  FROM meses\n  WHERE mes < '2024-12-01'\n)\nSELECT\n  m.mes,\n  COALESCE(COUNT(t.id), 0) AS num_transacciones,\n  COALESCE(SUM(t.monto), 0) AS monto_total\nFROM meses m\nLEFT JOIN transacciones t\n  ON strftime('%Y-%m', t.fecha) = strftime('%Y-%m', m.mes)\n  AND t.estado = 'Completada'\nGROUP BY m.mes\nORDER BY m.mes;" },
          { type: 'warning', title: 'Limitaciones de CTEs recursivos', content: 'En SQLite, los CTEs recursivos funcionan pero hay un limite por defecto de 1000 iteraciones. Para analisis de datos normales esto es mas que suficiente. En produccion, usa CTEs recursivos con precaucion y siempre incluye una condicion de parada clara.' },
        ],
        exercises: [
          {
            id: 'e-4-1-1',
          ...
          {
            id: 'e-4-1-2',
            instruction: 'Construye un analisis de portafolio de prestamos usando multiples CTEs. CTE 1 (resumen_cliente): para cada cliente, calcula numero de prestamos, monto total aprobado y saldo pendiente total. CTE 2 (clasificacion): agrega una columna "nivel_deuda" basada en saldo pendiente (>50M: "Critico", >20M: "Alto", >5M: "Medio", resto: "Bajo"). SELECT final: muestra nombre, num_prestamos, saldo_pendiente, nivel_deuda. Ordena por saldo pendiente DESC. Limita a 15.',
          ...
          {
            id: 'e-4-1-3',
            instruction: 'Usa un CTE recursivo para generar una serie de meses de 2024-01 a 2024-12. Luego haz LEFT JOIN con transacciones completadas para mostrar cada mes, el numero de transacciones y el monto total (usa COALESCE para meses sin datos). Ordena por mes.',
      ...
      },
      {
        ...
        content: [
          ...
          { type: 'theory', title: 'Analisis de cohortes paso a paso', content: '<strong>Paso 1: Definir la cohorte.</strong> Agrupa clientes por una caracteristica comun. Ejemplo: mes en que abrieron su cuenta (strftime("%Y-%m", fecha_apertura)).<br><br><strong>Paso 2: Medir comportamiento.</strong> Para cada cohorte, calcula metricas como numero de transacciones, monto total, etc.<br><br><strong>Paso 3: Comparar cohortes.</strong> Compara las metricas entre cohortes para identificar tendencias.<br><br><strong>Paso 4: Sacar conclusiones.</strong> Los clientes que abrieron cuenta en cierto periodo son mas activos? Hay estacionalidad?' },
        ...
        exercises: [
          ...
          {
            id: 'e-4-2-2',
            instruction: 'Realiza un analisis de cohortes: agrupa los clientes activos por el mes de apertura de su cuenta (strftime("%Y-%m", fecha_apertura) como cohorte). Para cada cohorte, muestra: numero de clientes, numero total de transacciones completadas (via cuentas y transacciones), y transacciones promedio por cliente. Ordena por cohorte.',
            initialQuery: '-- Analisis de cohortes por mes de apertura\n',
            hints: [
          ...
          {
            id: 'e-4-2-3',
            instruction: 'Analiza la actividad de clientes por canal de transaccion (App, Web, Sucursal, ATM, POS). Para cada cliente activo, determina su canal mas usado (el canal con mas transacciones). Muestra: canal_principal, numero de clientes que lo prefieren, y el promedio de transacciones totales de esos clientes. Usa CTEs y window functions.',
            initialQuery: '',
            hints: [
    ...
    lessons: [
      {
        ...
        content: [
          ...
          { type: 'theory', title: 'Paso a paso: Identificar entidades y relaciones del e-commerce', content: 'Vamos a aplicar el proceso de modelado ER al caso de TiendaOnline.co:<br><br><strong>Paso 1 - Identificar entidades:</strong> Del requerimiento extraemos: Categoria, Proveedor, Producto, Cliente, Pedido, DetallePedido, Resena, Cupon. Cada una representa un objeto del negocio.<br><br><strong>Paso 2 - Definir atributos clave:</strong><br>- productos: id, nombre, precio, costo, stock, sku, peso_kg, activo, fecha_creacion<br>- clientes: id, nombre, email, ciudad, departamento, telefono, tipo, fecha_registro, total_compras, puntos_fidelidad<br>- pedidos: id, fecha, estado, metodo_pago, subtotal, descuento, impuesto, total<br><br><strong>Paso 3 - Mapear relaciones:</strong><br>- categorias (1) --> (N) productos: cada producto pertenece a una categoria<br>- proveedores (1) --> (N) productos: cada producto tiene un proveedor<br>- clientes (1) --> (N) pedidos: cada pedido pertenece a un cliente<br>- pedidos (N) <--> (M) productos: relacion muchos a muchos resuelta con detalle_pedidos<br>- productos (1) --> (N) resenas y clientes (1) --> (N) resenas' },
          { type: 'code-example', title: 'Diagrama ER del e-commerce', content: 'Representacion textual del modelo ER de TiendaOnline.co con sus relaciones y cardinalidades:', code: "-- DIAGRAMA ER DE TIENDAONLINE.CO\n-- ================================\n--\n-- categorias (1) ----< (N) productos\n--   PK: id                  PK: id\n--   nombre                  nombre, precio, costo, stock\n--   descripcion             FK: categoria_id, proveedor_id\n--   margen_objetivo         sku, peso_kg, activo\n--\n-- proveedores (1) ----< (N) productos\n--   PK: id\n--   nombre, pais, calificacion, tiempo_entrega_dias\n--\n-- clientes (1) ---< (N) pedidos (1) ---< (N) detalle_pedidos\n--   PK: id             PK: id                 PK: id\n--   nombre, email      FK: cliente_id          FK: pedido_id\n--   ciudad, tipo       fecha, estado           FK: producto_id\n--   total_compras      metodo_pago, total      cantidad, subtotal\n--\n-- productos (1) ----< (N) detalle_pedidos  [resuelve N:M]\n-- productos (1) ----< (N) resenas\n-- clientes  (1) ----< (N) resenas\n-- cupones (entidad independiente: codigo, tipo, valor, fechas)\n\n-- Verificar las tablas del esquema real:\nSELECT name FROM sqlite_master\nWHERE type = 'table' ORDER BY name;" },
          { type: 'tip', title: 'Notacion de cardinalidad en diagramas', content: 'En diagramas ER profesionales se usa notacion de pata de gallo (crow foot). La regla de oro: la FK siempre va en el lado "muchos" de la relacion. Por eso <code>categoria_id</code> esta en productos (muchos productos por categoria), no al reves.' },
        ],
        exercises: [
          {
            id: 'e-5-1-1',
      ...
      },
      {
        ...
        exercises: [
          ...
          {
            id: 'e-5-2-3',
            instruction: 'Crea una tabla "valoraciones_proveedores" con: id (PK), proveedor_id (FK a proveedores, NOT NULL), puntuacion (INTEGER NOT NULL, CHECK entre 1 y 10), fecha_evaluacion (TEXT, DEFAULT fecha actual), criterio (TEXT, CHECK que sea "Calidad", "Precio" o "Entrega"), comentario (TEXT). Inserta 2 valoraciones para el proveedor 1 y muestralas.',
            initialQuery: '-- Tabla de valoraciones con CHECK y DEFAULT\n',
            hints: [
              'Usa CHECK (puntuacion >= 1 AND puntuacion <= 10) y CHECK (criterio IN (...))',
              "DEFAULT (date('now')) para la fecha automatica",
              "CREATE TABLE valoraciones_proveedores (id INTEGER PRIMARY KEY, proveedor_id INTEGER NOT NULL, puntuacion INTEGER NOT NULL CHECK (puntuacion >= 1 AND puntuacion <= 10), fecha_evaluacion TEXT DEFAULT (date('now')), criterio TEXT CHECK (criterio IN ('Calidad', 'Precio', 'Entrega')), comentario TEXT, FOREIGN KEY (proveedor_id) REFERENCES proveedores(id)); INSERT INTO valoraciones_proveedores (id, proveedor_id, puntuacion, criterio, comentario) VALUES (1, 1, 9, 'Calidad', 'Excelente calidad de componentes'); INSERT INTO valoraciones_proveedores (id, proveedor_id, puntuacion, criterio, comentario) VALUES (2, 1, 7, 'Entrega', 'A veces demora 1-2 dias mas'); SELECT * FROM valoraciones_proveedores;",
            ],
            requiredKeywords: ['CREATE TABLE', 'CHECK', 'DEFAULT'],
            solutionQuery: "CREATE TABLE valoraciones_proveedores (id INTEGER PRIMARY KEY, proveedor_id INTEGER NOT NULL, puntuacion INTEGER NOT NULL CHECK (puntuacion >= 1 AND puntuacion <= 10), fecha_evaluacion TEXT DEFAULT (date('now')), criterio TEXT CHECK (criterio IN ('Calidad', 'Precio', 'Entrega')), comentario TEXT, FOREIGN KEY (proveedor_id) REFERENCES proveedores(id)); INSERT INTO valoraciones_proveedores (id, proveedor_id, puntuacion, criterio, comentario) VALUES (1, 1, 9, 'Calidad', 'Excelente calidad de componentes'); INSERT INTO valoraciones_proveedores (id, proveedor_id, puntuacion, criterio, comentario) VALUES (2, 1, 7, 'Entrega', 'A veces demora 1-2 dias mas'); SELECT * FROM valoraciones_proveedores;",
          },
        ],
      },
      {
    ...
    lessons: [
      {
        ...
        content: [
          ...
          { type: 'theory', title: 'Paso a paso: Crear y verificar un indice', content: 'Vamos a crear un indice paso a paso y verificar que funciona:<br><br><strong>Paso 1 - Identificar la consulta lenta:</strong> Busca consultas con WHERE, JOIN u ORDER BY frecuentes. Ejemplo: filtrar transacciones por cuenta y fecha es muy comun en un banco.<br><br><strong>Paso 2 - Ver el plan SIN indice:</strong> Usa EXPLAIN QUERY PLAN para confirmar que hace SCAN TABLE (recorre toda la tabla).<br><br><strong>Paso 3 - Crear el indice:</strong> Elige las columnas en el orden correcto. En indices compuestos, la columna mas selectiva va primero.<br><br><strong>Paso 4 - Verificar con EXPLAIN:</strong> Ejecuta EXPLAIN QUERY PLAN de nuevo. Debe decir SEARCH USING INDEX en vez de SCAN TABLE.<br><br><strong>Paso 5 - Confirmar indices existentes:</strong> Consulta sqlite_master para ver todos los indices de la tabla.' },
          { type: 'code-example', title: 'Ejemplo completo: Crear y verificar indice', content: 'Paso a paso con transacciones bancarias:', code: "-- Paso 1: Consulta que queremos optimizar\n-- Buscar transacciones por cuenta y rango de fechas\n-- SELECT * FROM transacciones\n-- WHERE cuenta_origen_id = 5 AND fecha > '2024-06-01';\n\n-- Paso 2: Ver plan SIN indice\nEXPLAIN QUERY PLAN\nSELECT * FROM transacciones\nWHERE cuenta_origen_id = 5 AND fecha > '2024-06-01';\n-- Resultado: SCAN TABLE transacciones (lento)\n\n-- Paso 3: Crear indice compuesto\nCREATE INDEX idx_trans_cuenta_fecha\nON transacciones(cuenta_origen_id, fecha);\n\n-- Paso 4: Ver plan CON indice\nEXPLAIN QUERY PLAN\nSELECT * FROM transacciones\nWHERE cuenta_origen_id = 5 AND fecha > '2024-06-01';\n-- Resultado: SEARCH TABLE USING INDEX (rapido)\n\n-- Paso 5: Confirmar indices\nSELECT name, sql FROM sqlite_master\nWHERE type = 'index' AND tbl_name = 'transacciones'\nAND sql IS NOT NULL;" },
        ],
        exercises: [
          {
            id: 'e-6-1-1',
          ...
          {
            id: 'e-6-1-3',
            instruction: 'Crea un indice compuesto en la tabla prestamos sobre (cliente_id, estado) para optimizar consultas de prestamos por cliente y estado. Luego usalo: muestra todos los prestamos activos del cliente_id = 3, incluyendo tipo, monto_aprobado, saldo_pendiente y tasa_anual. Finalmente, verifica con EXPLAIN QUERY PLAN que el indice se esta usando.',
            initialQuery: '-- Indice compuesto en prestamos\n',
            hints: [
      ...
      },
      {
        ...
        content: [
          ...
          { type: 'theory', title: 'Paso a paso: Leer un plan de ejecucion', content: 'El plan de ejecucion te dice EXACTAMENTE como el motor SQL va a buscar tus datos. Aprende a leerlo:<br><br><strong>Paso 1 - Ejecutar EXPLAIN QUERY PLAN:</strong> Ponlo antes de cualquier SELECT para ver el plan sin ejecutar la consulta.<br><br><strong>Paso 2 - Buscar SCAN vs SEARCH:</strong><br>- <code>SCAN TABLE nombre</code> = recorre TODA la tabla fila por fila (lento en tablas grandes)<br>- <code>SEARCH TABLE nombre USING INDEX idx</code> = usa un indice para ir directo (rapido)<br>- <code>USING COVERING INDEX</code> = el indice tiene TODOS los datos necesarios, ni siquiera necesita ir a la tabla (optimo)<br><br><strong>Paso 3 - Identificar operaciones costosas:</strong> Si ves SCAN en una tabla grande, necesitas un indice. Si ves TEMP B-TREE para ORDER BY, un indice en esa columna lo eliminaria.<br><br><strong>Paso 4 - Crear el indice y comparar:</strong> Ejecuta EXPLAIN de nuevo despues de crear el indice. El plan debe cambiar de SCAN a SEARCH.' },
          { type: 'code-example', title: 'Lectura detallada de un plan', content: 'Veamos como leer un plan con JOIN:', code: "-- Plan para una consulta con JOIN\nEXPLAIN QUERY PLAN\nSELECT t.fecha, t.monto, cl.nombre\nFROM transacciones t\nJOIN cuentas cu ON t.cuenta_origen_id = cu.id\nJOIN clientes_banco cl ON cu.cliente_id = cl.id\nWHERE t.tipo = 'Transferencia'\n  AND t.monto > 1000000;\n\n-- El plan mostrara algo como:\n-- SCAN TABLE transacciones AS t\n-- SEARCH TABLE cuentas AS cu USING INTEGER PRIMARY KEY\n-- SEARCH TABLE clientes_banco AS cl USING INTEGER PRIMARY KEY\n--\n-- Interpretacion:\n-- 1. Escanea TODA transacciones (no hay indice en tipo+monto)\n-- 2. Para cada fila, busca en cuentas por PK (rapido)\n-- 3. Para cada cuenta, busca en clientes_banco por PK (rapido)\n-- Cuello de botella: el SCAN de transacciones" },
        ],
        exercises: [
          {
            id: 'e-6-2-1',
      ...
      },
      {
        ...
        content: [
          ...
          { type: 'theory', title: 'Paso a paso: Optimizar una consulta lenta', content: 'Sigamos un proceso sistematico para optimizar una consulta lenta:<br><br><strong>Paso 1 - Identificar la consulta problematica:</strong> En un banco, la consulta mas frecuente es buscar transacciones recientes de un cliente por canal.<br><br><strong>Paso 2 - Analizar con EXPLAIN:</strong> Ejecutar EXPLAIN QUERY PLAN para ver donde esta el cuello de botella. Normalmente sera un SCAN TABLE.<br><br><strong>Paso 3 - Eliminar SELECT *:</strong> Seleccionar solo las columnas necesarias reduce I/O.<br><br><strong>Paso 4 - Agregar indice estrategico:</strong> Crear indice en las columnas del WHERE y ORDER BY.<br><br><strong>Paso 5 - Reescribir JOINs ineficientes:</strong> Si hay subconsultas correlacionadas, convertirlas a JOIN. Si hay N+1 queries, consolidar en uno solo.<br><br><strong>Paso 6 - Verificar mejora:</strong> Ejecutar EXPLAIN de nuevo y confirmar que cambio de SCAN a SEARCH.' },
          { type: 'code-example', title: 'Optimizacion completa de una consulta', content: 'De una consulta lenta a una optimizada:', code: "-- ANTES: Consulta lenta (3 anti-patrones)\n-- 1. SELECT * (trae todo)\n-- 2. Subconsulta correlacionada (N+1)\n-- 3. Sin indice\n\n-- SELECT * FROM transacciones t\n-- WHERE t.cuenta_origen_id IN (\n--   SELECT id FROM cuentas\n--   WHERE cliente_id = (\n--     SELECT id FROM clientes_banco WHERE nombre = 'Carlos Mendez'\n--   )\n-- );\n\n-- DESPUES: Consulta optimizada\n-- 1. Solo columnas necesarias\n-- 2. JOINs en vez de subconsultas\n-- 3. Con indice\nSELECT t.fecha, t.tipo, t.monto, t.canal, t.estado\nFROM transacciones t\nJOIN cuentas cu ON t.cuenta_origen_id = cu.id\nJOIN clientes_banco cl ON cu.cliente_id = cl.id\nWHERE cl.nombre = 'Carlos Mendez'\nORDER BY t.fecha DESC\nLIMIT 20;\n\n-- El JOIN es mas eficiente que subconsultas anidadas\n-- porque el optimizador puede elegir el mejor plan" },
          { type: 'code-example', title: 'N+1 vs JOIN: El anti-patron mas comun', content: 'El problema N+1 ocurre cuando haces 1 query para listar + N queries para detalles:', code: "-- ANTI-PATRON N+1 (pseudocodigo):\n-- Query 1: SELECT id FROM sucursales (10 resultados)\n-- Query 2: SELECT COUNT(*) FROM clientes_banco WHERE sucursal_id = 1\n-- Query 3: SELECT COUNT(*) FROM clientes_banco WHERE sucursal_id = 2\n-- ... (10 queries adicionales!)\n\n-- SOLUCION: Un solo JOIN con GROUP BY\nSELECT\n  s.nombre AS sucursal,\n  s.ciudad,\n  COUNT(cl.id) AS num_clientes,\n  ROUND(AVG(cl.ingresos_mensuales), 0) AS ingreso_promedio\nFROM sucursales s\nLEFT JOIN clientes_banco cl ON s.id = cl.sucursal_id\nGROUP BY s.id, s.nombre, s.ciudad\nORDER BY num_clientes DESC;\n-- 1 solo query en vez de 11!" },
        ],
        exercises: [
          {
            id: 'e-6-3-1',
          ...
          {
            id: 'e-6-3-3',
            instruction: 'Resuelve un problema N+1: en vez de consultar cada sucursal individualmente, escribe UNA sola consulta con JOIN que muestre para cada sucursal: nombre, ciudad, numero de clientes activos, numero de cuentas totales, y suma total de saldos. Ordena por suma de saldos descendente.',
    ...
    lessons: [
      {
        ...
        content: [
          ...
          { type: 'theory', title: 'Paso a paso: Transaccion con verificacion en cada etapa', content: 'Vamos a construir una transaccion bancaria completa paso a paso, verificando el estado en cada etapa para entender como funciona la atomicidad.<br><br><strong>Paso 1:</strong> Verificar saldos ANTES de operar.<br><strong>Paso 2:</strong> Iniciar la transaccion y debitar.<br><strong>Paso 3:</strong> Verificar que el debito fue correcto.<br><strong>Paso 4:</strong> Acreditar y registrar la operacion.<br><strong>Paso 5:</strong> Verificar saldos DESPUES y confirmar con COMMIT.' },
          ...
          { type: 'code-example', title: 'Pasos 2-5: Transaccion completa con verificaciones', content: 'Ejecutamos la transaccion verificando en cada paso:', code: "-- Paso 2: Iniciar transaccion y debitar\nBEGIN;\nUPDATE cuentas SET saldo = saldo - 750000\nWHERE id = 1 AND saldo >= 750000;\n\n-- Paso 3: Verificar debito (dentro de la transaccion)\nSELECT id, saldo FROM cuentas WHERE id = 1;\n\n-- Paso 4: Acreditar cuenta destino\nUPDATE cuentas SET saldo = saldo + 750000 WHERE id = 5;\n\n-- Paso 5: Verificar ambas cuentas y confirmar\nSELECT id, saldo FROM cuentas WHERE id IN (1, 5);\nCOMMIT;" },
          { type: 'code-example', title: 'SAVEPOINT: Puntos de guardado parciales', content: 'SAVEPOINT permite deshacer parte de una transaccion sin cancelarla completa:', code: "-- SAVEPOINT permite rollback parcial\nBEGIN;\n\nUPDATE cuentas SET saldo = saldo - 200000 WHERE id = 2;\nSAVEPOINT despues_debito;\n\nUPDATE cuentas SET saldo = saldo + 200000 WHERE id = 99;\n-- Ups! Deshacemos SOLO esta operacion\nROLLBACK TO despues_debito;\n\nUPDATE cuentas SET saldo = saldo + 200000 WHERE id = 3;\nCOMMIT;\nSELECT id, saldo FROM cuentas WHERE id IN (2, 3);" },
          { type: 'tip', title: 'Cuando usar SAVEPOINT', content: 'SAVEPOINT es ideal cuando una transaccion tiene multiples pasos y quieres manejar errores parciales sin perder todo el trabajo previo. En banca, se usa para reintentar operaciones fallidas dentro de la misma transaccion.' },
        ],
        exercises: [
          {
            id: 'e-7-1-1',
          ...
          {
            id: 'e-7-1-3',
            instruction: 'Simula una operacion bancaria multi-paso usando SAVEPOINT: Inicia una transaccion, debita 300,000 de la cuenta 2, crea un SAVEPOINT, intenta acreditar 300,000 a la cuenta 2 misma (error logico), haz ROLLBACK TO al savepoint, acredita 300,000 a la cuenta 4, y confirma. Verifica saldos de cuentas 2 y 4.',
            initialQuery: '-- Operacion multi-paso con SAVEPOINT\nBEGIN;\n',
            hints: [
              'Usa SAVEPOINT nombre_punto; despues del debito',
      ...
      },
      {
        ...
        content: [
          ...
          { type: 'theory', title: 'Ejemplos practicos de aislamiento', content: 'Veamos como se comporta el aislamiento con un ejemplo concreto del banco. Dentro de una transaccion, siempre ves tus propios cambios, pero otros usuarios no los ven hasta el COMMIT.' },
          { type: 'code-example', title: 'Lecturas consistentes dentro de transaccion', content: 'Dentro de una transaccion, tus lecturas reflejan tus propias escrituras:', code: "-- Verificamos que nuestra transaccion ve sus propios cambios\nBEGIN;\n\n-- Leemos el saldo original\nSELECT id, saldo FROM cuentas WHERE id = 1;\n\n-- Hacemos un deposito\nUPDATE cuentas SET saldo = saldo + 500000 WHERE id = 1;\n\n-- Nuestra transaccion VE el nuevo saldo\nSELECT id, saldo FROM cuentas WHERE id = 1;\n\n-- Pero si hacemos ROLLBACK, todo se revierte\nROLLBACK;\n\n-- El saldo original se mantiene\nSELECT id, saldo FROM cuentas WHERE id = 1;" },
          { type: 'code-example', title: 'Tabla de aislamiento vs problemas', content: 'Resumen de que previene cada nivel:', code: "-- Tabla resumen (conceptual)\nSELECT\n  'READ UNCOMMITTED' AS nivel, 'Ninguno' AS previene,\n  'Permite dirty reads, non-repeatable, phantom' AS problemas\nUNION ALL SELECT\n  'READ COMMITTED', 'Dirty Reads',\n  'Permite non-repeatable reads, phantom reads'\nUNION ALL SELECT\n  'REPEATABLE READ', 'Dirty + Non-Repeatable',\n  'Permite phantom reads'\nUNION ALL SELECT\n  'SERIALIZABLE', 'Todos',\n  'Mas lento pero totalmente seguro';" },
        ],
        exercises: [
          {
            id: 'e-7-2-1',
          ...
          {
            id: 'e-7-2-2',
            instruction: 'Demuestra la consistencia transaccional: dentro de una transaccion, haz dos depositos de 250,000 a la cuenta 3 (dos UPDATEs separados), luego verifica que el saldo aumento exactamente 500,000. Haz ROLLBACK al final para no alterar los datos.',
            initialQuery: '-- Consistencia: dos depositos en una transaccion\n',
            hints: [
      ...
      },
      {
        ...
        content: [
          ...
          { type: 'code-example', title: 'SQL Injection: Ejemplos educativos', content: 'Veamos como funciona SQL injection y como prevenirlo:', code: "-- EJEMPLO: Login bypass\n-- Codigo vulnerable: query = 'SELECT * FROM users WHERE user=' + input\n-- Si el atacante ingresa: ' OR '1'='1' --\n-- Resultado: retorna TODOS los usuarios\n\n-- EJEMPLO: Extraccion de datos via UNION\n-- Un atacante puede inyectar UNION SELECT para\n-- extraer datos de otras tablas (cedulas, emails)\n\n-- PREVENCION: Siempre usar parametros preparados\n-- PostgreSQL: SELECT * FROM users WHERE email = $1\n-- MySQL: SELECT * FROM users WHERE email = ?\n-- Python: cursor.execute('SELECT ... WHERE email = %s', (email,))\n\n-- Verificar datos sensibles en nuestro banco\nSELECT COUNT(*) AS total_clientes FROM clientes_banco;" },
    ...
    lessons: [
      {
        ...
        exercises: [
          ...
          {
            id: 'e-8-1-3',
            instruction: 'Analisis de engagement por cohorte usando la tabla reproducciones: para cada mes de registro del usuario (cohorte), calcula el total de reproducciones, el promedio de minutos vistos por reproduccion, y el porcentaje de reproducciones completadas. Usa JOIN entre reproducciones y usuarios.',
      ...
      },
      {
        ...
        content: [
          ...
          { type: 'theory', title: 'Paso a paso: Funnel AARRR con SQL', content: 'Vamos a construir un funnel AARRR completo para StreamCo, midiendo cada etapa del ciclo de vida del usuario.<br><br><strong>Paso 1:</strong> Acquisition - cuantos se registraron.<br><strong>Paso 2:</strong> Activation - cuantos vieron al menos 1 contenido.<br><strong>Paso 3:</strong> Retention - cuantos siguen activos.<br><strong>Paso 4:</strong> Revenue - cuantos tienen plan de pago.' },
          { type: 'code-example', title: 'Funnel AARRR completo', content: 'Construimos el funnel midiendo la conversion entre cada etapa:', code: "WITH funnel AS (\n  SELECT\n    COUNT(*) AS total_registrados,\n    SUM(CASE WHEN contenidos_vistos > 0 THEN 1 ELSE 0 END) AS activados,\n    SUM(CASE WHEN estado = 'Activo' THEN 1 ELSE 0 END) AS retenidos,\n    SUM(CASE WHEN estado = 'Activo' AND plan_actual != 'Basico' THEN 1 ELSE 0 END) AS monetizados\n  FROM usuarios_streaming\n)\nSELECT\n  total_registrados AS acquisition,\n  activados AS activation,\n  ROUND(activados * 100.0 / total_registrados, 1) AS pct_activacion,\n  retenidos AS retention,\n  ROUND(retenidos * 100.0 / activados, 1) AS pct_retencion,\n  monetizados AS revenue,\n  ROUND(monetizados * 100.0 / retenidos, 1) AS pct_monetizacion\nFROM funnel;" },
        ],
        exercises: [
          {
            id: 'e-8-2-1',
          ...
          {
            id: 'e-8-2-3',
            instruction: 'Analiza el engagement por dispositivo: usando la tabla reproducciones, muestra para cada dispositivo el total de reproducciones, el promedio de minutos vistos, el porcentaje de contenidos completados, y el numero de usuarios distintos. Ordena por total de reproducciones descendente.',
            initialQuery: '-- Engagement por dispositivo\n',
            hints: [
              'GROUP BY dispositivo en la tabla reproducciones',
              'COUNT(DISTINCT usuario_id) para usuarios unicos por dispositivo',
